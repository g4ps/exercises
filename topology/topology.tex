...
If all $a_i$'s are equal to $0$, then we follow that $\ran(h) = \set{(b_1, b_2, ...)}$.
We follow that if $(b_1, b_2, ...) \in B_\rho(x, \epsilon)$, then
$h\inv[B_\rho(x, \epsilon)] = R^\omega$. If $(b_1, b_2, ...) \notin B_\rho(x, \epsilon)$,
then $h\inv[B_\rho(x, \epsilon)] = \emptyset$. Thus we conclude that if $a_n$ is all
zeroes, then $h$ is still continous. 
Thus we follow that a conjecture that $a_n$ need not to
converge to 0 is false.
Since $\ran(h) = \set{(b_1, b_2, ...)}$, we follow that $h$ is not a bijection and thus it cannot
be a homeomorphism.

Now we can conjecture something useful: in order for $h$ to be continous $a_n$ and $b_n$ can
be arbitrary and in order for $h$ to be a homemorphism, there should be no $a_n$ equaling zero.
...
thus we can follow that if none of $a_n$ are equal to zero, then $h$ is bijective.

\textbf{IDEA}: $a_i$'s ought to be bounded. This gives us continuity for all zeroes and prevents this
thing from creatign a strand in inverse image of a basis element. If $a_i$'s are bounded, then
we can make a proof by deconstructing $B(x, \epsilon)$ using identity from previous exercise.
Then every $x$ in inverse image is surrounded by some space. If $a_i$'s are unbounded, then
inverse image has a strand around $(b_1, b_2, ...)$ so that no basis element fits in there.

TODO: expand this idea to a full-blown proof.

We also have that if some $a_i$'s are zeros, then function is not bijective, otherwise it is.
We might want to have bounded set $\set{\frac{1}{a_i}}$ on the account that we want to impose
equal restraint for continuity for reverse image.

$b_i$'s don't matter whatsoever.
