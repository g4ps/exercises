...
$E$ is a somewhat curious case, but after some thought we can follow that there is
no interval/ray in both $Y$ and $R$ that contains $0$ and does not contain a point in $1/x$
for some $x \in Z_+$, as was proven in the previous batch of exercises, when we've proven
that $R_k$ is strictly finer than the standart topology of $R$.
...
Suppose that $q \in X \times Y$ is open. Then we follow that $q = X' \times Y'$, where
$X'$ is an open set in $X$ and $Y'$ is an open set in $Y$. Therefore we follow that
$$\pi_1(q) = X_1$$
$$\pi_2(q) = Y_1$$
both of which are open.
Thus we follow that if $q$ is an open set in $X \times Y$, then $\pi_1(q)$ and $\pi_2(q)$ are
both open as well, which means that $\pi_1$ and $\pi_2$ are both open maps, as desired.
