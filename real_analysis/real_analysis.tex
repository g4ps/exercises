...
If $B_2$ is finite, then let $n = |B_2|$. Then we can have function
$$f: N \to A_1 \cup B_2: $$
$$f(x) = \text{ x'th smallest element of } B_2 \text{ if }x \leq n $$
$$f(y) = g(y - n) $$
where $g$ is a function, that maps $N \to A_1$ (which exists, because the
set $A_1$ is countable).

N'th smallest element in this case is defined to be $x \in A: \forall k \in A
x \geq k$, if $n = 1$ or $x \in A:  \in A: x > j$, where $j$ is (N-1)'th
smallest element.
