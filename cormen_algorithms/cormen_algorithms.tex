...
I still don't get the specificity that we go into, when we deal with base cases for
our induction procedures. And this goes back to the definition of the algrorithmic recurrences
that we've been given before: how in heck can a function $T(n)$ not be
in $\Theta(1)$ for $n < n_0$ for some $n_0 \in N$? Since the function is defined
on natural numbers, we follow that $n < n_0$ is a compact set, thus $T(n)$ has a minimum
and maximum value, which implies that for given values
$$T(n) \in \Theta(g)$$
for any $g: R \to R$. And even if we don't take into account compactness and all that stuff,
I still don't understand how exactly can a function not be in $\Theta(g)$ for given
values, given that definition of $\Theta(g)$ specifies exactly that its restrictions
are only applied for a large enough $n$.
...
Suppose that for all $m < n$ we've got that
$$T(m) \in O(m\lg(m))$$
thus there exists $c \in R$ such that
$$T(m) \leq c m\lg(m))$$
We firstly follow that for all $n$
$$\lg(n/2 + 17) = \lg(n(1/2 + 17/n)) = \lg(n) + \lg(1/2 + 17/n)$$
and thus for $n > 34$ we've got that 
we follow that
$$T(n) = 2T(n/2 + 17) + n \leq c (n/2 + 17) \lg (n/2 + 17) + n \leq $$
now we use the point, that we've shown earlier
$$ c (n/2 + 17) (\lg(n) + \lg(1/2 + 17/n)) + n \leq$$
use some algebra
$$ c n/2 (\lg(n) + \lg(1/2 + 17/n)) + 17 c (\lg(n) + \lg(1/2 + 17/n)) + n \leq$$
applying the fact that $cn/2 = cn - cn/2$
$$ c n (\lg(n) + \lg(1/2 + 17/n)) - c n/2 (\lg(n) +
\lg(1/2 + 17/n)) + 17 c (\lg(n) + \lg(1/2 + 17/n)) + n \leq$$
using some more algebra
$$ c n (\lg(n) + \lg(1/2 + 17/n)) + (17 c - cn/2) (\lg(n) + \lg(1/2 + 17/n)) + n \leq$$
shifting some elements around
$$ c n (\lg(n) + \lg(1/2 + 17/n)) -  (cn/2 - 17 c) (\lg(n) + \lg(1/2 + 17/n)) + n \leq$$
then assuming that $n > 4$, which implies that $\lg(n) > 2$ and in turn
$\lg(n) + \lg(1/2 + 17/n) > \lg(n) + \lg(1/2) > 2 - 1 \geq 1$ by the fact that $\lg$
is monotonically increasing
$$ c n (\lg(n) + \lg(1/2 + 17/n)) -  (cn/2 - 17 c) + n \leq$$
then doing some asymptotics magic and assuming that $n$ is large enough (larger than
34 to be precise)
$$ c n (\lg(n) + \lg(1/2 + 17/n)) \leq$$
and then assuming that $n > 34$ once again, which implies that $17/n < 1/2$, which implies that
$1/2 + 17/n < 1$ and thus $\lg(1/2 + 17/n) < 0$
$$ c n \lg(n)$$

\textit{TODO: do the rest of the exercises in this chapter}

\chapter{Probabilistic Analysis and Randomized Algorithims}

\section{The hiring problem}

\subsection*{5.1-1}

\textit{Show that the assumption that you are always able to determine which candidate
  is best, in line 4 of procedure Hire-Assistant, implies that you know a total order on the
  ranks of the candidates.}

Idk what exactly do I need to prove in here, given that partial order is pretty self-explanatory and
the ability to compare any two elemetnts is assumed as well, given the randomness of the order in
the input.

\subsection*{5.1-2}

\textit{Describe an implementation of the procedure $Random(a, b)$ that makes calls only to
  $Random(0, 1)$. What is expected running time of your procedure, as a function of $a$ and $b$?}

Let's firstly assume that we want the uniform distribution of the numbers between $a$ and $b$ here.
Then we can follow that we can simplify the given task to creating a function, that
returns a random number from $0$ to $a - b$. We can follow that we also can create a function
from $0$ to some power of $2$ by determining "bits" from our original function. We can conclude
that given function will have running time of $\lg(a - b)$ in case $a - b$ is a power of two.
If $a - b$ is not a power of $2$, then we can use a function that creates a random
number in span of power of $2$, then throwing this number out in case our resulrting
number is greater than $a - b$ and starting over untill the resulting number is less or equal
to $a - b$. Thus we conclude that creating a number that is exactly a power of $2$
will happen in $\lceil \lg(a - b) \rceil$, and given that expected number of runs to
produce the number in the correct span is $2$, we follow that the resulting algorithm will
have a running time of $2 \lceil \lg(a - b) \rceil$. Our discussions in the third chapter allow
us to conclude that the asymptotic expected  running time of this algorithm
will be $\Theta(\lg(a - b))$
