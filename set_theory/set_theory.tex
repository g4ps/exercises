...
If $\beta \in \On$, then $G(\beta) = \sup\set{G(\eta): \eta \in \beta}$, and

Suppose that $x \in F(G(\beta))$. We follow that $x \in \sup\set{F(\eta): \eta \in G(\beta)}$.
Thus $x \in \bigcup\set{F(\eta): \eta \in G(\beta)}$

$$x \in F(G(\beta)) \lra x \in \sup\set{F(\eta): \eta \in G(\beta)}
\lra (\exists c \in \set{F(\eta): \eta \in G(\beta)})(x \in c) \lra$$
$$ \lra
(\exists \eta \in G(\beta))(c = F(\eta) \land x \in c) 
$$
